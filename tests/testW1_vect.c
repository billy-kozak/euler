...
*                             FUNCTION PROTOTYPES                             *
******************************************************************************/
static bool mstf_test_intRandWR(enum w1_indexMode im,size_t ns,bool updateInd);
static bool mstf_test_intRandWR_iter(enum w1_indexMode im,size_t ns);
/******************************************************************************
*                                    TESTS                                    *
******************************************************************************/
/**
...
static bool mstf_test_intRandWR(enum w1_indexMode im,size_t ns,bool updateInd){
	...
	unsigned seed;
	bool ret = true;
	struct w1_vect* v = w1vect_init(im,ns,INTEGER);
...
int main(int argc, char** argv){
	...
	MSTF_START();
		...
		MSTF_RUN(
				...
				"cache enabled",
				mstf_test_intRandWR(AUTO,0,false)
			);
		MSTF_RUN(
				"Failed integer write readback with "
				"cache in manual mode",
				mstf_test_intRandWR(MANUAL,0,false)
			);
		MSTF_RUN(
				"Failed integer write readback with "
				"cache disabled",
				mstf_test_intRandWR(OFF,0,false)
			);
		MSTF_RUN(
				"Failed integer write iterator readback with "
				"cache disabled",
				mstf_test_intRandWR_iter(OFF,0)
			);
		MSTF_RUN(
				"Failed integer write iterator readback with "
				"cache disabled",
				mstf_test_intRandWR_iter(AUTO,0)
			);
		MSTF_RUN(
				"Failed integer write readback with "
				"cache in manual mode and a cache update",
				mstf_test_intRandWR(MANUAL,0,true)
			);
	MSTF_END();
